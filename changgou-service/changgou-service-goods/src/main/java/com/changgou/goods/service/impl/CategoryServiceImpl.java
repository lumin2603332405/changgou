...
public class CategoryServiceImpl implements CategoryService {
    ...
    public List<Category> findByParentId(Integer pid) {
        ...
        Category condition = new Category();//条件对象
        condition.setParentId(pid);//  parent_id=?
//        condition.setId(1);// and id=1

        return categoryMapper.select(condition);//条件查询  条件一定都是=号条件  exmpale ：是任意的条件
